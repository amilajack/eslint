...
Trailing commas in function declarations and function calls are valid syntax since EcmaScript® 2017. So the string option does not check function declarations and function calls for backward compatibility.

You can also use a separate option for each type of syntax.
Each of the following options can be set to `"never"`, `"always"`, `"always-multiline"`, `"only-multiline"`, or `"ignore"`.
Default is `"never"` basically.
